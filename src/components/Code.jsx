...
function CodePanelHeader({ tag, label }) {
  ...
  return (
    <div className="flex h-9 items-center gap-2 bg-white px-4 dark:border-b-white/5 dark:bg-white/1 hidden">
      {tag && (
  ...
  return (
    <div className="group dark:bg-white/2.5">
      ...
      <div className="relative">
        <pre
          className="overflow-x-auto p-4 text-md font-serif text-slate-800 dark:text-white"
...
function CodeGroupHeader({ title, children, selectedIndex }) {
  ...
  return (
    <div className="flex min-h-[calc(theme(spacing.12)+1px)] flex-wrap items-start gap-x-4 px-4  bg-blue-600  dark:bg-blue-600/30  dark:ring-1 dark:ring-blue-400/20">

 
      
      {title && (
        <h3 className="mr-auto pt-3 text-xs font-semibold text-white dark:text-white-400">
          {title}
        </h3>
      )}
      {hasTabs && (
            ...
            <Tab
              className={clsx(
                ...
                childIndex === selectedIndex
                  ...
                  : 'border-transparent text-zinc-200 hover:text-zinc-300'
              )}
            >
...
export function CodeGroup({ children, title, ...props }) {
  ...
  return (
    <CodeGroupContext.Provider value={true}>
      <Container
        {...containerProps}
        className="not-prose my-6 text-black overflow-hidden rounded-2xl bg-white shadow-md dark:bg-zinc-900 dark:ring-1 dark:ring-white/10"
      >
...
export function Pre({ children, ...props }) {
  ...
  return (
    <CodeGroup {...props}>
      <pre className="font-serif" style={{ fontFamily: 'Times New Roman' }}>
        {children}
      </pre>
    </CodeGroup>
  );
